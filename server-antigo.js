...
app.post("/clientes", async (req, res) => {
  try {
    ...
    if (origem.toLowerCase() === "typebot") {
      ...
      // ✅ Se veio grupoId, pega o preço do grupo (preco_venda)
      ...
      if (grupoFinalId) {
        ...
        // ✅ valor do pedido = preco_venda do grupo * quantidade
        valorTotalFinal = precoVendaGrupo * quantidade;

        // opcional: usa o nome do grupo como "produto" exibido (mantém seu campo produto)
        produtoFinal = normalizeString(grupo.nome) || produtoFinal || "Grupo";
      }

      // fallback se ainda não ficou válido (mantém compat com seu padrão antigo)
      ...
      const clienteTypebot = {
        ...
        // nova data (vai existir quando virar Vendido)
        data_venda: "",
    ...
    // 🟡 CHECKOUT
    // ✅ Se vier grupoId, calcular valor_total pelo preco_venda do grupo * quantidade
    ...
    if (grupoCheckoutId) {
      ...
      // opcional: usa nome do grupo como "produto" (mantém seu campo produto)
      produtoCheckout = normalizeString(grupo.nome) || produtoCheckout || "Grupo";
    }

    // fallback de segurança (compat)
    ...
    if (statusEnvioFinal === "Etiqueta Gerada") {
      ...
      if (!cliente.financeiro_frete_id) {
        const saida = {
          tipo: "Saída",
          categoria: "Frete",
          descricao: `Envio do cliente ${cliente.nome}`,
          valor: valorEnvioFinal,
          data: dataEnvioFinal,
          origem: "Loggi",
          cliente: { id, nome: cliente.nome, phone: cliente.phone },
          criado_em: new Date().toISOString(),
        };

        const saidaRef = await db.collection("financeiro").add(saida);
        sync.financeiro_frete_id = saidaRef.id;
      }
    }
